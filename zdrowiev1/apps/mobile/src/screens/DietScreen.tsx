...
export const DietScreen = () => {
  ...
  const handleAddMeal = () => {
        ...
        ? [
            {
              ...
              carbs: product.carbs,
              fat: product.fat,
              quantity: 100, // default placeholder
            },
          ]
        : [
            {
    ...
    setNewMealName('');
    cancelScan(); // clear barcode product after adding
  };
  ...
  if (isScanning) {
    return <BarcodeScanner onBarcodeScanned={handleBarcodeScanned} onClose={cancelScan} />;
  }
    ...
    <ScrollView className="flex-1 bg-background p-6">
      ...
      <GlassCard entering={FadeInUp.delay(200).springify()} intensity={20}>
        ...
        <TouchableOpacity
          ...
          className="bg-secondary py-4 rounded-xl items-center flex-row justify-center active:opacity-80"
          onPress={scanBarcode}
        >
          <Text className="text-white font-bold">📸 Skanuj kod kreskowy</Text>
        </TouchableOpacity>

        {product && (
          <View className="mt-4 p-4 bg-muted/20 rounded-xl">
            <Text className="text-foreground font-medium">Znaleziono: {product.name}</Text>
            <Text className="text-muted-foreground text-xs">{product.calories} kcal / 100g</Text>
          </View>
        )}

        {error && <Text className="text-destructive mt-2 text-center">{error}</Text>}
      </GlassCard>
